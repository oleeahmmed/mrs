  ...
  return (
    <div className="relative w-[226px] h-[226px]">
      ...
      >
        ...
        {segments.map((segment, index) => (
          <React.Fragment key={segment.id}>
            <div
              className={`absolute font-inter font-semibold text-[6.5px] tracking-[0] leading-normal ${segment.id === 1 ? 'w-[34px] top-[7px] left-0' :
              ...
              style={{ color: `var(--color-text-${segment.textColor})` }} // Use CSS variable for text color
            >
              {segment.text}
            </div>
            {segment.image && (
      ...
      <button
        ...
        className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-16 h-16 rounded-full text-black font-bold text-sm z-10 ${isSpinning || spinsAvailable <= 0 ? 'opacity-50 cursor-not-allowed' : 'hover:brightness-110 cursor-pointer'}`}
        style={{ background: 'var(--color-table-header)' }} // Use table-header gradient
      >
        {isSpinning ? 'Spinning...' : 'SPIN'}
      </button>
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-4 h-8 z-20">
        <div className="w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-b-[16px]" style={{ borderBottomColor: 'var(--color-primary)' }}></div>
      </div>
      {winner && !isSpinning && (
        <div className="absolute top-[-40px] left-1/2 transform -translate-x-1/2 text-black font-bold py-1 px-3 rounded-md text-sm" style={{ background: 'var(--color-table-header)' }}>
          You won: {winner.text}!
        </div>
      )}
    </div>
